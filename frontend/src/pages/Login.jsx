  ...
  return (
    <form className='min-h-[80vh] flex items-center'>
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg'>
        ...
        <button className='bg-[#40E0D0] text-white w-full py-2 rounded-md text-base'>{state === 'Sign Up' ? "Create Account" : "Login"}</button>
        {
          state === "Sign Up" ?
            <p>Already have an account? <span onClick={() => setState('Login')} className='text-[#40E0D0] underline cursor-pointer'>Login here</span></p> :
            <p>Create new account? <span onClick={() => setState('Sign Up')} className='text-[#40E0D0] underline cursor-pointer'>Click Here</span></p>
        }
      </div>
    </form>
  )
